...
def make_little_distributed_iqs(
    ...
    int0_write_cap: int = 2,
    int1_write_cap: int = 1,
    mem_write_cap: int = 3,
    div_write_cap: int = 1,
    fpsimd_write_cap: int = 3,
    div_pool=None,
    fpsimd_pool=None,
...
class LittleV052Rv64Core(BaseCPUCore):
    ...
    def __init__(
        ...
        dist_int0_write_cap: int = 2,
        dist_int1_write_cap: int = 1,
        dist_mem_write_cap: int = 3,
        dist_div_write_cap: int = 1,
        dist_fpsimd_write_cap: int = 3,
        ...
        # so this single control propagates to:
        ...
        # RV64IMAFD scalar FP state uses the floating-point register
        # class. Keep the vector physical-register budget aligned with
        # the frozen proxy for future RVV experiments; RVV is not part
        # of the initial cross-ISA replication gate.
...
class LittleV052Rv64Processor(BaseCPUProcessor):
    def __init__(
        self,
        rob_entries: int,
        iq_entries: int,
        ...
        if div_raw_wake and not pair_shared_div:
            raise ValueError(
                "--div-raw-wake requires --pair-shared-div"
            )

        if div_raw_wake and not div_reactive_power:
            raise ValueError(
                "--div-raw-wake requires --div-reactive-power"
            )
        ...
        shared_div_pool = (
            LittlePairSharedDivPool(
                ...
                rawPredictiveWakeEnabled=div_raw_wake,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Little Core v0.52 RV64 cross-ISA replication proxy"
    )
    parser.add_argument("--binary", required=True, help="RV64 static ELF path")
    parser.add_argument(
        "--program-arg",
        action="append",
        default=[],
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        help=(
            "Per-core RV64 static ELF for multi-core SE; "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Use raw RV64 IntDiv/Rem predecode classification to wake the "
            "pair-shared DIV domain before full architectural decode"
        ),
    )
    parser.add_argument(
        "--div-idle-threshold",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "1 is conservative for RV64 binaries that may contain RVC; "
            "2 is for independently verified 32-bit-only code"
        ),
    )
    ...
    parser.add_argument(
        "--checker",
        action="store_true",
        help="Enable strict RV64 O3 CheckerCPU validation",
    )
    parser.add_argument(
        "--distributed-iq",
        action="store_true",
...
def main() -> None:
    ...
    processor = LittleV052Rv64Processor(
        ...
        div_raw_wake=args.div_raw_wake,
    ...
    print(
        ...
        f"div-raw-wake={args.div_raw_wake}",
